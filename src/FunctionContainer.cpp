...
MemoryCell FunctionContainer::getFunction(const std::string& id, const std::vector<MemoryCell>& params) const 
{
    ...
    for (const auto& [paramId, value] : std::ranges::views::zip(func.getParams(), params)) {
        _handler->allocate(paramId, value);
    }
    MemoryCell value = func.getValue();
